...
const DraggableShape: React.FC<DraggableShapeProps> = ({ element, canvasRef, isSelected, onSelect, onUpdate }) => {
    ...
    const handleDragMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        if (shapeRef.current) {
            elementStartPos.current = { x: shapeRef.current.offsetLeft, y: shapeRef.current.offsetTop };
        }
    };
    ...
    const handleResizeMouseDown = (e: React.MouseEvent<HTMLDivElement>, handle: string) => {
        ...
        if (canvasRef.current && shapeRef.current) {
    ...
    useEffect(() => {
        const handleMouseMove = (e: MouseEvent) => {
            ...
            if (isResizing) {
                ...
                if (newWidth < 10) { if (resizeHandle.includes('w')) newX += newWidth - 10; newWidth = 10; }
                if (newHeight < 10) { if (resizeHandle.includes('n')) newY += newHeight - 10; newHeight = 10; }
    ...
    const renderShape = () => {
        const { shape, color } = element;
        const style = { backgroundColor: color };
        switch(shape) {
            case 'rectangle': return <div className="w-full h-full" style={style} />;
            case 'circle': return <div className="w-full h-full rounded-full" style={style} />;
            default: return null;
        }
    };
